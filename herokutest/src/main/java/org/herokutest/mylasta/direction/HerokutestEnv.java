 ...
 * @author FreeGen
 */
public interface HerokutestEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/herokutest */
    ...
    /** The key of the configuration. e.g. returnpath@herokutest.org */
    String MAIL_RETURN_PATH = "mail.return.path";

    /** The key of the configuration. e.g. herokutest-support@annie.example.com */
    String MAIL_ADDRESS_SUPPORT = "mail.address.support";

    /** The key of the configuration. e.g. org.h2.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:h2:file:$classes(org.herokutest.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/herokutestdb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. herokutestdb */
    ...
    /** The key of the configuration. e.g. localhost:8090/herokutest */
     ...
     * The value is, e.g. /tmp/lastaflute/herokutest <br>
     ...
     * The value is, e.g. returnpath@herokutest.org <br>
     ...
     * The value is, e.g. herokutest-support@annie.example.com <br>
     * comment: Mail Address for Herokutest Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. jdbc:h2:file:$classes(org.herokutest.dbflute.allcommon.DBCurrent.class)/../../etc/testdb/herokutestdb <br>
     ...
     * The value is, e.g. herokutestdb <br>
     ...
     * The value is, e.g. localhost:8090/herokutest <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements HerokutestEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(HerokutestEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(HerokutestEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(HerokutestEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(HerokutestEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(HerokutestEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(HerokutestEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(HerokutestEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(HerokutestEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(HerokutestEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(HerokutestEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(HerokutestEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(HerokutestEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(HerokutestEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(HerokutestEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(HerokutestEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(HerokutestEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(HerokutestEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(HerokutestEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(HerokutestEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(HerokutestEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(HerokutestEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(HerokutestEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(HerokutestEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(HerokutestEnv.SERVER_DOMAIN);
        }
    }
}
